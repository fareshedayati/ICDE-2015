...
\section{Induce decision trees on sparse input data} \label{sec:sparse-input-dt}
...
sampled at each node. Furthermore, we hope to speed up the overall
algorithm by taking into account the input space sparsity. Compressed sparse
column matrix already satisfy the fast column indexing requirement. We are
going to show how to efficiently exploit the data structure  as to have fast
row indexing and speed up the overall algorithm on sparse data.

Given the sparse matrix format, the main issue is to efficiently perform the
extraction of the feature value reaching the node (the line \ref{alg-line:value-extract}
of Algorithm \ref{algo:find-best-split}). Otherwise said for a
given feature $j$, one have to be able to perform the intersection between the
sample set $\mathcal{L}$ which have reached the node and the $m_j=indptr[j+1] -
indptr[j]$  non zero element of the feature $j$ as to generate the set of
possible splitting rule.  Once this is done, we will be able to speed up the
split search by treating separately splitting rule with positive and negative
thresholds. If we assume that the $indices$ of the input csc matrix
array are sorted per column, then standard intersection
algorithms have the following time complexity:
\begin{enumerate}
\item in $O(|\mathcal{L}| \log{m_j})$ by performing $|\mathcal{L}| $ binary
search on the sorted $m_j$ non zero elements;
\item in $O(|\mathcal{L}|\log{|\mathcal{L}|} + m_j\log{|\mathcal{L}|})$
by sorting the sample set $\mathcal{L}$ and performing $m_j$ binary search on
$\mathcal{L}$;
\item in $O(|\mathcal{L}|\log{|\mathcal{L}|} + m_j + |\mathcal{L}|)$ by sorting the
sample set $\mathcal{L}$ and retrieve the intersection by iterating over both
array;
\item in $O(m_j + |\mathcal{L}|)$ by first creating a temporary hash table from the
smallest array and then checking if elements of the second array are contained
in the hashtable.
\end{enumerate}

The approach developed in this paper allow to perform the intersection
between any column of the csc input sparse matrix and the sample set
$\mathcal{L}$ in a time complexity which is faster than the hash table
approach.
...
S &= n\_samples  \times \log(n\_samples) \label{S} \\
B_f &= n\_samples  \times \log(n\_indices_f) \label{B}
\end{align}
...
\If{$samples[start:end]$ is sorted}
	...
	\State Conduct a binary search
	\Else
	\State  Conduct a reverse index search
	\EndIf
\Else
