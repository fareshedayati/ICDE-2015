...
In practice, the number of non zero element $m_j$ of features $j$
could be a lot bigger than the size of a sample set $\mathcal{L}_p$. For instance,
 this is likely to happen near the leaves nodes whenever tree are fully developed.
There is only a few   samples reaching those nodes. For optimal performance,
one can use a hybrid intersection algorithm which combines the previously
developed mapping approach to approach (1) based on binary search. whenever
$\mathcal{L}_p \ll m_j$, the binary approach is likely to be faster.

In the context of sparse data, one more optimization that could be made is
to remember which features are constant. During the tree growth, this will
avoid the overhead of searching for a best split where none exists.
