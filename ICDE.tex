...
that can be represented in a sparse input space. State-of-the-art tree-based ensemble
algorithms cannot scale to such datasets, since they include operations whose running time 
is a function of the input space size rather than a function of the non-zero input elements. %and
%implementations have focused on tasks with dense input space, while at most
%emulating random memory access on sparse input data. 
In this paper, we propose an
efficient splitting algorithm to leverage input sparsity within decision tree
methods. %We exploit the a-priori knowledge that each column has very few non-
%zero elements and show how learning time is significantly decreased.
Our algorithm improves training time over sparse datasets by more than
two orders of magnitude and it will be incorporated in the next version
of \emph{scikit-learn}, the most popular open source Python machine learning library.
...
given a split on feature $j$ in $O(m_j)$ instead of $O(n)$. For more details of this algorithm refer to Algorithm~\ref{algo:extract_nnz_mapping}. 
...
m_j$, the binary approach will be faster. For more details of this algorithm refer to Algorithm~\ref{algo:extract_nnz_bsearch}. 

The hybrid algorithm switches between Algorithms ~\ref{algo:extract_nnz_mapping} and ~\ref{algo:extract_nnz_bsearch} by the following rule:
 \begin{equation}
 \label{switch}
 (1- sorted) \times |\mathcal{L}_p| \times \log(|\mathcal{L}_p|) + |\mathcal{L}_p| \times \log(m_j) < 0.1 m_j
 \end{equation}
where $sorted$ is $1$ if $\mathcal{L}_p$ is sorted and $0$ otherwise.  Algorithm~\ref{algo:extract_nnz_bsearch} is used whenever Equation~\ref{switch} is true and Algorithm~\ref{algo:extract_nnz_mapping} is used otherwise. 
...
\begin{algorithm}\label{algo:extract_nnz_mapping}
Extract nonzero values of the current node via $mapping$, and partition the positive and negative values.  
%\textnormal{
\begin{algorithmic}[1]
    ...
    \For{$k \in [indptr[j] \mbox{:} indptr[j+1]]$}
        ...
        \State $value = data[k]$
         ...
         \State $h$ = $mapping[index]$
            	\If {$value > 0$}
		\State $positives.append(value)$
		\State \Call{Swap}{$\mathcal{L}$,  $j$, $start_p$}
		\State $mapping[\mathcal{L}[h]] = h$
		\State $mapping[\mathcal{L}[start_n]] = start_p$
	         \Else
	         \State $negative.append(value)$
	         \State \Call{Swap}{$\mathcal{L}$,  $j$,$end_n$}
		\State $mapping[\mathcal{L}[h]] = h$
		\State $mapping[\mathcal{L}[end_n]] = end_n$
	        \State $end\_n \,\, += 1$
	        \EndIf
...
\begin{algorithm}\label{algo:extract_nnz_bsearch}
Extract nonzero values of the current node by a binary search, and partition the positive and negative values. 
%\textnormal{
\begin{algorithmic}[1]
\Function{extract\_nnz\_bsearch}{$\mathcal{L}$, $X$, $mapping$, $j$, $start$, $end$, $sorted$}
    ...
    \EndIf 
     \For{$h \in [start:end]$}
	   ...
	   \Comment{\#Returns the position of $index$ in  $indices_j$, -1 if not found.}
	   \If {$i \neq -1$ }
	   	 \If{$data_j[i]  > 0$}
		\State $positives.append(value)$
		\State \Call{Swap}{$\mathcal{L}$,  $h$, $start_p$}
		\State $mapping[\mathcal{L}[j]] = h$
		\State $mapping[\mathcal{L}[start_n]] = start_p$
	         \Else
	         \State $negative.append(value)$
	         \State \Call{Swap}{$\mathcal{L}$,  $j$,$end_n$}
...
\caption{Leveraging the input sparsity significantly speed up decisions
         tree induction both with shallow and deep trees on the \emph{adult}
         dataset.}
\label{fig:adult}
\end{figure}
...
\caption{Leveraging the input sparsity significantly speed up decisions
         tree induction both with shallow and deep trees on the \emph{tic}
         dataset.}
\label{fig:tic}
\end{figure}
