...
efficiently with sparsely representable data, efficient matrix formats have been
...
allows large speed up. It decreases sorting time of samples in the current node along features which is an essential component in all tree-based models. Moreover it reduces the set of possible splits to evaluate at
...
introduces decision tree splitting algorithm and sparse matrix format; Section
\ref{sec:sparse-input-dt} describe the proposed splitting algorithm for sparse
...
random-forests sections \cite{buitinck2013api,pedregosa2011scikit}.
...
be consistent with \emph{scikit-learn} \cite{buitinck2013api,pedregosa2011scikit}.
...
\Function{InduceDecisionTree}{$X$, $Y$}
    \State Initialize tree structure $\tau$ with root node $t_0$
    \State Initialize an empty stack $stack$
    \State Initialize a sample set $\mathcal{L}=\{1,\ldots,n\}$
    \State $stack$\Call{.push}{($t_0$, $\mathcal{L}$)}
    \While{$stack$ is not empty}
        \State $t_p$, $\mathcal{L}_p$ = $stack$\Call{.pop}{}()
        \If{$t_p$ satisfies stopping criterion}
            \State Make $t_p$ a leaf node using $\mathcal{L}_p$.
        \Else
            \State \begin{varwidth}[t]{0.8\linewidth}
                   Find a split $s^*$ which maximize impurity reduction
                   among possible splitting rules $Q(\mathcal{L}_p, X)$:
                   \[
                   s^* = \arg\max_{s \in Q(\mathcal{L}_p, X)} \Delta{}I(s,X,Y).
                   \]
                   \end{varwidth}
            \State Make $t_p$ an internal node given split $s$.
...
\Function{FindBestSplit}{$\mathcal{L}$,$X$, $Y$}
    ...
    \For{$j \in \{1, \ldots, p\}$}
        ...
        \State Sort $\mathcal{L}$ and $\mathcal{X}_j$ by increasing values
               of $\mathcal{X}_j$.
        \State Generate possible splitting rules
               \[
        ...
        \For{$s$ in $Q(\mathcal{X}_j)$}
            \State Evaluate impurity reduction of splitting rule $s$
                   \[
                   \text{score} = \Delta{}I(s,X, Y).
                   \]
            \If{$\text{score} > \text{best}$}
                \State $\text{score} =\text{best}$
                \State $s^* = s$
            \EndIf
        \EndFor
    \EndFor
