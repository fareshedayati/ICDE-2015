...
called compressed sparse column (csc) matrix format. It is general format to
...
of each non zero elements and an $indptr$ array which stores the beginning of
each columns in the $data$ and the $indices$ arrays.

For instance, the matrix $3 \times 5$ matrix
\[
\begin{bmatrix}
1 & 0 & 0 & 4 & 0 \\
0 & 0 & 0 & 5 & 0 \\
...
\end{bmatrix}
\]
is compactly represented by a csc matrix with the following arrays
\begin{align*}
...
The main advantages of csc matrix is to allow fast column slicing, efficient
arithmetic and matrix operations. However, row slicing is slower. A similar
row-based sparse matrix called compressed sparse row format also exists and
works under similar principles.
