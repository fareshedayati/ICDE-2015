...
learning package doesn't support sparse input for tree-based methods, are
restricted to decision stump, decision tree with only one internal node or have
sub-optimal implementation through the simulation of random access memory as in
the dense case. The only solution is often to densified the input space which
leads first to severe memory constraints and then to very slow training time.
...
allows large speed up by reducing the number of possible split to evaluate at
each node. We also want to highlight that the contribution of this paper have
been proposed to inclusion to the  \emph{scikit-learn}
\cite{buitinck2013api,pedregosa2011scikit} open source package in order to make
profit to the machine learning community.
