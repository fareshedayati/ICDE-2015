...
between an input space $\mathcal{X}$, where each dimension is called a feature
to some output categorical or numerical variable $\mathcal{Y}$. While those
datasets have very high dimensional input space, they are often sparsely
representable. For instance, the number of unique words associated to a text
document is actually small compared to the all words of a given language. In
order to work efficiently with such data, efficient matrix formats have been
developed with fast operations, such as dot product, and a low memory
footprints.
