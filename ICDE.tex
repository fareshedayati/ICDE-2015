...
The main advantages of csc matrices are to allow fast column slicing, efficient
arithmetic and matrix operations. However, row slicing is slow. Note that a
...
almost evenly \cite{joachims1996probabilistic}. Training data was carried on with
different parameters of $max\_depth$ and the training data was represented once in a $csc\_matrix$ sparse matrix
and once with a dense matrix. As  Figure~\ref{depth} shows,
the training time with a densely represented matrix is much higher than the in their sparsely represented
counterparts. We verified that all $4$ fitted trees were identical.

The second set of tests were carried on synthetic data. Random matrices were
generated with $n\_samples = 100000$, and $n\_features = 1000$  and with varying densities. Density is the percentage
of non-zero values of a feature. For example a density of $0.1$ means that
every feature is only $10\%$ of the time non-zero. For each density we created $10$ random matrices. Each matrix was represented
both in a sparse and dense format. Decision trees were fitted once with the
sparse and once in the dense format. Figure~\ref{density} contains the training
time of these matrices. As it can be seen in this figure
the training time of sparse matrices is much lower that their dense
counterparts when the density of matrix is quite low, i.e. less $0.2$. For not-
quite sparse matrices, e.g. $density = 0.5$, the training time with dense
inputs is lower than their sparse counterparts. As this figure suggests
