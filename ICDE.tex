...
indptr[j]$  non zero elements of the feature $j$ as to generate the set of
possible splitting rules.  Once this is done, we will be able to speed up the
split search by treating separately splitting rules with positive and negative
...
sample set $\mathcal{L}$ and retrieving the intersection by iterating over both
arrays;
...
Nevertheless in the context of a decision tree, it's possible to perform the
intersection between any column of the csc input sparse matrix and the sample
set $\mathcal{L}$ faster than the hash table approach.



Depending on the size of these two sets we can either
